...
class Settings(BaseSettings):
    ...
    # Секретный ключ для подписи запросов/проверки уведомлений (из ЛК Продамуса).
    prodamus_secret_key: str = ""
    # Куда Продамус вернёт пользователя после оплаты. Если пусто — соберётся
    ...
    # Как часто опрашивать статус pending-платежей (минуты) — подтверждение оплаты
    # выбрано опросом статуса (у заказчицы нет вебхука на этом этапе).
